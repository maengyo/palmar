...
WSLg 에서 창 제목줄은 **윈도우가 그린다.** palmar 쪽에서 두께를 바꿀 수 없고, 그 아래에 palmar 가
그리는 44px 바가 또 있어서 캔버스가 그만큼 줄어든다. 제목줄을 아예 끄면 그 높이를 돌려받는다:

```sh
./target/release/palmar-app --no-titlebar
```

그러면 palmar 의 상단 바가 제목줄 노릇을 한다 — 끌어서 옮기고, 더블클릭으로 최대화하고,
오른쪽에 최소화·최대화·닫기 버튼이 생긴다.

**기본값이 아니라 옵션이다.** 제목줄이 없는 창은 페이지가 요청해 주지 않으면 옮길 수 없으므로,
거기서 뭔가 잘못되면 **옵션을 빼고 다시 띄우는 것**이 빠져나오는 길이다.
