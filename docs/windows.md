...
- **이름도 같은 길로 온다 (2026-09-21).** 윈도우에는 **앞 프로세스가 없다** — ConPTY 에 프로세스
  그룹이 없고 `GetConsoleProcessList` 는 부르는 쪽이 그 콘솔에 붙어 있어야 한다. 그래서
  `foreground_pid()` 가 늘 `None` 이고, pane 이름이 전부 `shell` 이었다(사용자, 2026-09-21).
  폴더와 똑같이 **셸이 직접 말한다**: `PSConsoleHostReadLine` 래퍼가 133;C 를 내면서
  `palmar:run:<앞의 세 낱말>` 을 같이 쓰고, 프롬프트가 `palmar:run:` 을 빈 채로 써서 지운다.
  한 번이라도 말한 pane 에서는 `sample_fg` 가 물러난다 — 안 그러면 다음 틱이 `None` 으로 덮는다.
- **세 낱말인 이유, 그리고 나오는 것이 이름이어야 하는 이유.** 이름 하나에, 앞이 인터프리터일 때
  뒤의 스크립트까지가 필요한 전부다. 나머지는 남의 명령줄이고, 명령줄은 명령만 들어 있지 않다 —
  `$env:TOKEN="abc"; aelix` 가 그 첫 낱말을 통째로 레일에 올리고 있었다(시험이 잡았다, 야생에서가
  아니라). 이제 보이는 이름은 `NAMEISH` 를 통과해야 한다 — 글자와 파일이름이 가지는 문장부호뿐이고,
  아니면 아무 말도 안 한다. 또 따옴표를 아는 채로 쪼개는데, 공백으로 자르면
  `"C:\Program Files\...\git.exe"` 가 `Program` 이 된다(이것도 시험이 잡았다).
...
이름 쪽은 `tests/test_pure.py` 의 `TheShellSaysWhatItRuns` 아홉 개다.

> **아직 윈도우에서 안 재 봤다.** 위의 이름 길은 전부 맥에서 — 손으로 같은 표식을 pane 에
> 흘려보내는 것으로 — 확인했다. 머리말의 PowerShell 자체는 이 기계에서 못 돌린다.
