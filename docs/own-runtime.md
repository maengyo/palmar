...
| 시나리오 | fps | 프레임 p95 | 최악 | 긴 작업 | 키→화면 p50 | 처리량 | WebGL |
|---|---|---|---|---|---|---|---|
...
| pane 8 + **전부** 최대 출력 | 60 | 18.7ms | 18.8ms | 0 | 3.3ms | **30MB/s** | 8 |
...
1. **흐름 제어가 있으면 브라우저는 밀리지 않는다.** ACK 를 준 만큼만 오기 때문에 브라우저가
   자기 속도를 정한다. 그래서 "부하를 주면 버벅이나" 가 아니라 "60fps 를 지키면서 얼마나
   받나" 가 질문이 되고, 답이 pane 8 전부 출력에서 30MB/s 다. 서버 쪽 최대치(34MB/s)와
   거의 같다 — **브라우저가 병목이 아니었다.**
