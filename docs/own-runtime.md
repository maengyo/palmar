...
파이썬은 `loop.remove_reader` 로 **PTY 읽기를 진짜 멈춘다.** Bun 은 못 멈춘다 —
`Bun.Terminal` 에 pause 가 없다(조사에서 이슈 #41410 로 이미 나왔던 것이 실물로 확인됐다).
**정확히 하자면 Bun 스파이크는 보내기를 막아 보지도 않았다** — `paused` 플래그를 세우기만 하고
`ws.send` 는 그대로 나간다. 막았더라도 PTY 는 계속 읽히므로 메모리에 쌓이는 것은 같다.
