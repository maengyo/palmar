...
**화면은 셋이다.** 왼쪽에서 폴더를 고르면 그 경로의 셸이 캔버스에 열리고, 그 안에서 무엇을
돌릴지는 사용자가 정한다. 오른쪽 목록은 어느 터미널이 나를 기다리는지 보여 준다 —
창이 겹쳐 있어도, 화면 밖에 있어도.

```
┌─ 왼쪽 ────┬─ 캔버스 ───────────────┬─ 오른쪽 ──┐
│ 디렉터리  │ 터미널이 놓아둔 자리에 │ 터미널    │
│ 폴더만 ↻  │ 그 크기로. 겹쳐도 되고 │ 목록      │
│ 게으르게  │ 새 것은 빈 자리에      │ 기다리는  │
│ 읽는다    │ 앉는다                 │ 것이 위로 │
└───────────┴───────────┬────────────┴───────────┘
                        │ 웹소켓 (연결 수는 ① 에서 정한다)
┌───────────────────────┴────────────────────────┐
...
- **palmer** (지금): TUI 를 중계하는 대신 브라우저가 직접 그린다. 에뮬레이션은 xterm.js 로 넘기고,
  ...
  산다(#21 이 그린 모양). **셋 다 설계일 뿐 아직 재지 않았다** (`docs/own-runtime.md` 의 "아직 안 잰 것").
...
브라우저가 기본이고 이유는 **줌**이다. **줌이 두 종류인데 둘 다 브라우저에서만 된다.**
`Ctrl+-` 는 **밀도 줌**이다 — 셀이 작아져 터미널마다 행·열이 늘고, 캔버스에 더 들어온다.
캔버스의 +/- 버튼은 **카메라 줌**이다 — 화면만 축소되고 행·열은 그대로다.
...
**카메라 줌으로는 PTY 크기를 바꾸지 않는다** — 창 크기 조절만이 행·열을 바꾼다.
...
- **겹침은 허용하되 새 것은 빈 자리에 앉는다.** 끄는 설정이 있다
- **캔버스에 확대·축소 버튼이 있다** — 카메라 줌이라 터미널의 행·열은 안 바뀐다.
  브라우저 `Ctrl+-` 는 셀을 줄여 행·열을 늘리는 다른 줌이다
...
- **데몬의 언어.** Python(핸드오프 가능, 웹소켓 손수) 대 Bun(PTY·웹소켓 내장, 핸드오프 불가).
  **PTY → 웹소켓 → xterm.js 파이프라인을 둘로 만들어 같은 조건에서 재서 정한다**
...
- **카메라 줌을 어느 길로 할지.** cate 가 이 문제를 풀며 여덟 가지를 고쳤다 — 줌으로 PTY 를
  리사이즈하면 90열이 87열이 되고, 축소 뒤엔 터미널이 하얘진다. 세 갈래(계단 배율 + 좌표 고쳐 쓰기 /
  정확 추적 / WebGL 애드온 패치) 중 고른다
- **앱을 언제 낼지.** 앱을 내면 "설치 한 줄" 이 "받아서 실행" 으로 바뀐다
- xterm.js 8~16개 + WebGL 예산제의 브라우저 비용 — 사람이 봐야 하는 것 포함
