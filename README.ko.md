...
**없는 동안 무슨 일이 있었나.** 신호등은 지금 참인 것을 말하고, 오른쪽의 활동 목록은 그 사이에
무슨 일이 있었는지를 말한다 — 무엇이 끝났고 무엇이 물어봤고 어떤 차례였는지. **데몬이 갖고 있어서**
브라우저를 닫아 둔 동안도 남는다. 자리를 가장 오래 비우는 때가 바로 그때다.
마지막으로 창을 앞에 둔 뒤에 생긴 것은 따로 표시된다.

**미니맵**이 지금 보고 있는 캔버스의 것으로 오른쪽 아래에 있다. 누르거나 끌어서 보는 자리를 옮긴다.

**창을 움직이는 것은 사람뿐이다.** 맨 아래 터미널을 닫으면 캔버스가 알아서 줄지만, 맨 위를 닫으면
위쪽 빈 자리는 남는다 — 그것을 거두려면 창을 움직여야 하고, **남의 창이 닫혔다고 보고 있던 창이
뛰면 안 되기 때문이다.** 탭 줄의 단추가 시킬 때 해 준다: 서로의 간격을 지킨 채 전부 구석으로 당겨지고, 거둘 것이 없으면
...
| 보안 | 무언가를 바꾸는 요청마다 `0600` 파일의 토큰과 `Origin`·`Host` 검사. |
