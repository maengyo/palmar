...
주소를 찍고, 창을 열고 — palmar 의 창이 있으면 그것, 없으면 크로미엄 계열 브라우저의 앱 모드
(기본 브라우저가 크로미엄 계열이면 그것; 윈도우엔 Edge 가 늘 있다), 그것도 없으면 브라우저 탭 — **바로 돌아온다**. exe 없이 제 프로그램으로
만들려면 Edge 나 Chrome 에서 한 번 앱으로 설치한다(메뉴 → 앱 → palmar 설치): 자기 아이콘·창·시작
메뉴 항목이 생기고, 그 뒤로 `palmar` 는 그것을 연다 — 데몬은 떨어져
나가므로 터미널을 닫아도 셸이 같이 죽지 않는다. 다시 `palmar` 를 치면 두 번째 데몬이 아니라 같은 주소를 준다. `palmar --stop` 이 끝낸다.
