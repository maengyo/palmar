...
![palmar: 캔버스 위의 터미널 셋. 각각 불이 하나씩 — 일하는 중, 나를 기다림, 끝남. 그중 둘을 끌어다 이름 붙은 그룹으로 묶으면 같이 움직이고, ⌥ 를 누른 채 끌면 하나가 빠져나온다. PDF 가 자기 창으로 열리고, Ctrl+휠로 물러나면 판 전체가 한눈에 들어온다](docs/img/palmar.gif)
...
잠깐 올려두고 있으면 함께 움직이는 **그룹**이 되고, 그룹에는 이름을 붙일 수 있다. 무엇을 하든
`Ctrl`/`⌘`+`Z` 한 번으로 되돌린다.

**그리고 판은 화면보다 넓다.** 버튼 둘이 정리한다 — 하나는 창들을 모서리로 데려오고, 다른 하나는
배치는 그대로 둔 채 간격만 좁힌다 — 빈 바닥 위에서 `Ctrl`+휠이면 다 보일 만큼 물러난다. 물러나는
것은 **그리기만** 작게 하는 것이라, 어떤 터미널에도 새 크기를 안 알리고 아무것도 다시 재지 않는다.
...
창은 같은 것을 브라우저 없이 보여 주는 **712 KB** 짜리 프로그램이다([`app/`](app/README.md)) — OS 가
이미 가진 웹뷰를 쓰고, Electron 이 아니다. (리눅스 내려받기 기준이고, 맥은 한 파일이 인텔·애플실리콘을
같이 실어서 1.5 MB 다.) 창이 있으면 `palmar` 가 창을 열고, 브라우저는 창 안의
