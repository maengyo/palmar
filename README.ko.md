...
> ⚠️ **아직 초기.** 이제 데몬(`server/palmerd.py`)과 브라우저 UI(`web/`)가 돈다 — 아래 **해 보기**.
> 이 문서는 여전히 **계획과, 코드 전에 확인한 사실**이고, 몇 가지는 사람이 정할 몫이다(웹 스택,
> 좌표 저장 위치, 색, codex 상태, 앱 출시 시점).
...
│  브라우저가 없어도 산다                        │
└──────┬─────────────────────┬───────────────────┘
       │ PTY (원시 바이트)   │ 훅 — URL 의 pane id 로 짝을 짓는다
       │ (입력·크기 포함)    │ (PATH shim 이 pane 마다 붙인다)
┌──────┴─────────────────────┴───────────────────┐
...
![palmer 가 도는 화면 — 캔버스 셋이 탭으로, 캔버스 위의 터미널들, 목록 맨 위의 기다리는 세션](docs/img/palmer-light.png)

*캔버스 셋이 탭으로 있고, `infra` 에 점이 붙어 있다 — 거기서 뭔가 답을 기다린다는 뜻이다. 왼쪽 목록은 캔버스로 거르지 않는다. 기다리는 것은 어느 캔버스에 있든 맨 위다. 오른쪽 아래는 지금 보는 캔버스의 미니맵이다.*

## 해 보기

```
python3 server/palmerd.py            # 마지막 줄에 http://127.0.0.1:8801 을 찍는다 — 그 주소를 연다
```

명령 하나, 인자 없음, 설치 없음: `/usr/bin/python3`(3.9.6)만 있으면 되고, 그건 `git` 이 있는 곳엔 있다.
`127.0.0.1` 에만 묶는다. 찍힌 주소를 브라우저로 연다.

**보이는 것:** 세 열 — 왼쪽 터미널 목록, 가운데 점 격자 캔버스, 오른쪽 디렉터리 레일. 오른쪽에서
폴더를 고르고 **Open terminal here** 를 누르면 그 경로에서 셸이 캔버스에 뜬다. 제목줄을 끌어 옮기고,
모서리를 끌어 크기를 바꾼다(셸의 행·열이 따라온다). 펼치기 상자를 누르면 하나를 캔버스 크기로 키우고,
**Esc** 또는 "back to canvas" 알약을 누르면 돌아온다 — 펼친 창이 없는 캔버스로 넘어가도 펼침이 풀린다.
탭을 닫고 주소를 다시 열어도 세션은 그대로다 (데몬이 PTY 를 갖고 브라우저는 붙기만 한다).

**캔버스 — 탭은 캔버스 위에 있다.** 원노트처럼 캔버스 위에 탭 줄이 붙는다. **＋** 로 새 캔버스를
만들면 바로 이름을 묻고, 나중에 이름을 바꾸려면 탭을 두 번 누르거나 F2 를 누른다. 탭을 끌면 순서가
바뀐다. 탭에는 **작은 점 하나**만 붙는다 — 그 캔버스에 나를 기다리는 것이 있다는 표시고, 그 이상은
안 붙는다. 쳐다본다고 점은 안 꺼진다(다음 훅이 꺼 준다). 열려 있는 브라우저 전부가 같은 탭 줄을
같은 순서로 본다 — 순서의 주인이 데몬이기 때문이다.
탭 줄은 그 안에서 손을 놀리는 동안에도 정직하다: 탭을 끄는 도중에 상태가 바뀌어도 끌던 탭이 흐트러지지
않고, 이름 고치는 칸이 열려 있어도 점은 계속 맞고, 화면 밖으로 밀려 있던 탭으로 넘어가면 그 탭을
줄 안으로 끌어다 놓는다.

**왼쪽 목록은 캔버스로 거르지 않는다.** 어느 캔버스에 있든 전부, 상태별로, 기다리는 것이 맨 위다.
다른 캔버스의 것에는 캔버스 이름표가 붙고, 누르면 그 캔버스로 넘어가 그 창으로 간다. 그룹 머리를
누르면 접히고 — **개수는 남는다** — 새로고침해도 접힌 채다. 찾는 중에는 접힘을 무시한다(맞는 것이
접힌 그룹에 숨지 않게).

**이름.** 캔버스와 터미널 둘 다 이름을 줄 수 있다. 창은 제목 옆 이름 상자, 또는 이름을 두 번 누르면
된다. 비우면 지워지고 경로 이름표로 돌아간다.

**미니맵.** 캔버스 오른쪽 아래, **지금 보고 있는 캔버스 것 하나**뿐이다. 창을 늘리는 만큼 캔버스가
한없이 자라므로 "이 안에서 내가 어디 있나" 에 답한다. 눌러서·끌어서 보는 자리를 옮긴다.

**아직 안 되는 것:**
- **캔버스를 지우는 단추도, 터미널을 다른 캔버스로 옮기는 손잡이도 없다.** 데몬은 둘 다 되고
  (`DELETE /api/canvases/<id>`, `PATCH /api/sessions/<id>`) 열려 있는 브라우저도 제대로 따라오지만,
  그 손잡이를 화면 어디에 두느냐는 아직 사람이 정할 것이라(⑪) 지어내지 않았다.
- **탭이 많아지면 ＋ 가 탭 줄 밖으로 밀린다.** 캔버스 20개면 줄이 상자보다 두 배 넘게 길고 스크롤바를
  감춰 뒀으므로, 가로로 밀기 전에는 ＋ 에 손이 안 닿는다. **현재 탭**은 줄 안으로 끌어다 놓지만
  ＋ 는 아니고, 넘칠 때의 규칙은 아직 안 정했다.
- **창에도 닫기 단추가 없다.** 셸에서 `exit` 를 치면 창이 사라진다(실측: 목록에서 빠지고 pane 설정
  파일도 지워진다). 브라우저는 DELETE 를 보내지 않는다 — 창은 세션을 보는 눈이고, 세션은 UI 보다 오래 산다.
- **데몬을 다시 띄우면 아무것도 안 남는다** — 캔버스도 이름도 셸도. ⑦=b(핸드오프 없음)와
  "디스크에 아무것도 안 쓴다" 의 결과고, `docs/protocol.md` 에 그렇게 적혀 있다.
- **zsh 에서는 훅이 붙는다. bash·fish 는 아직이다.** shim 을 `PATH` 맨 앞에 두는 것만으로는 졌다 —
  사용자 `.zshrc` 가 나중에 돌며 자기 것을 다시 앞에 세운다. 그래서 zsh 는 `ZDOTDIR` 로 감싸
  palmer 의 rc 가 **마지막에** 돌며 shim 을 되돌린다. 사용자 파일은 읽기만 한다.
  실측: pane 에서 진짜 `claude` 를 띄우니 3초 만에 `working` 이 왔고, 사용자는 아무것도 안 했다.
  **bash·fish 는 안 감쌌으므로** 그 셸에서는 점이 회색에 머물 수 있다.
- **창을 다른 창 위로 끌면 겹친다** — 밀어내기(#23)는 아직 없다.
- **JetBrains Mono 웹폰트를 받지 않는다**(런타임에 네트워크 없음): 로컬에 없으면 시스템 모노스페이스로 떨어진다.
- 설정 패널·⌘K 명령·codex/opencode 훅은 아직 없다(⑧ 미정).
...
- **훅은 대화상자와 같은 순간에 온다.** Claude Code 2.1.259 에서 승인 프롬프트가 화면에 뜬 것과
  ...
  알림이다(1회 관측 6.0초). 스파이크에서는 `--settings` 로 얹어 확인했고, 제품에서는 palmer 가
  여는 터미널마다 PATH shim 이 같은 일을 한다 — 사용자가 설치할 것이 없다(아래 "정해진 것").
...
- **훅은 스스로 붙는다.** palmer 가 여는 셸마다 PATH 맨 앞에 `~/.palmer/bin` 이 있고, 거기 두 줄짜리
  `claude` 껍데기가 진짜 `claude` 를 pane 별 `--settings` 파일과 함께 exec 한다. 사용자 설정은 안
  건드리고 설치할 것도 없다. **실측:** `--settings` 는 다른 층을 대체하지 않고 병합한다(스파이크 E).
  `command claude` 나 절대 경로는 껍데기를 비켜 간다 — 그때는 회색으로 두고 이유를 알려 준다.
  오전의 "한 번 설치" 결정을 뒤집었다 — 그 전제가 틀렸었다
...
- ~~데몬의 언어~~ — **2026-09-07 정했다: 파이썬, 바닥은 3.9.** 둘로 만들어 쟀는데 속도로는 안
  갈렸다. 갈린 것은 흐름 제어다 — 브라우저가 느릴 때 파이썬은 PTY 읽기를 멈춰 자식이 `write` 에서
  막히지만, Bun 은 그럴 방법이 없다(`Bun.Terminal` 이 pause 도 fd 도 안 준다. RSS 31MB 대 87MB).
  설치가 쐐기를 박았다 — `/usr/bin/python3` 이 3.9.6 이고 스파이크가 전부 거기서 돌아 내려받을 것이
  없다. 그 심은 `/usr/bin/git` 과 바이트 단위로 같아서, git 이 있는 기계엔 python3 도 있다.
  **되돌릴 수 있게 둔다** — 파이썬에만 있는 구조를 안 쓰고, 프로토콜을 문서로 고정하고,
  터미널 상태를 데몬 밖에 둔다. Bun 이 fd 를 열면 다시 본다
- ~~데몬 재시작 전략~~ — **2026-09-07 정했다: 재시작하지 않는다.** 업데이트 때만 다시 띄우고,
  에이전트는 `--resume`, 셸 pane 은 죽는다. 핸드오프는 만들어서 됐지만(스파이크 G) 안 골랐다 —
  언어가 파이썬으로 굳고 처음부터 그 모양으로 짜야 하기 때문이다
...
- [`README.md`](README.md) — 이 문서의 영어판. **저장소의 기본은 영어판이다**
...
**이 저장소의 README 와 커밋 메시지는 영어가 기본이다.** 이 한글판을 그와 함께 맞춰 둔다.
`docs/` 나머지는 한국어다 — 읽는 사람이 하나인 작업 문서이기 때문이다.

한글판이 영어판보다 뒤처지면 **영어판이 맞다.** 둘을 같이 고치거나, 못 고치면 여기에 적어 둔다.
