...
코딩 에이전트를 위한 공간 캔버스. 터미널은 놓아 둔 자리에, 준 크기 그대로 있고, 탭으로 넘나드는
캔버스 위에 얹힌다. 각 창은 **일하는 중**인지, **나를 기다리는지**, **끝났는지**, **한가한지**를 보여 준다 —
안에서 무엇이 도는지 따로 알려 주지 않아도 알아낸다.

브라우저 탭을 닫아도 세션은 계속 산다.

![palmer 실행 화면: 탭으로 늘어선 캔버스, 캔버스에 놓인 터미널, 목록 맨 위에 올라온 나를 부르는 세션](docs/img/palmer-light.png)

## 무엇을 하나

**터미널에 자리가 있고, 그 자리를 지킨다.** 오른쪽 레일에서 폴더를 고르고 *Open terminal here* 를
누르면 그 경로에 셸이 열린다. 제목 줄을 끌어 옮기고 모서리를 끌어 크기를 바꾸면 셸의 행·열이 따라간다 —
...
있을 때. 개수도 닫기 단추도 없다. 탭의 주인은 페이지가 아니라 데몬이라, 열려 있는 브라우저 모두가
같은 탭을 같은 차례로 본다.

**왼쪽 목록은 캔버스로 거르지 않는다.** 어디에 있든 돌고 있는 것이 전부 들어 있다. 캔버스로 묶이고
접히지만, **접힘이 내가 필요한 것을 감추는 일은 없다.** 기다리는 것이 있는 캔버스는 맨 위로 뜨고,
접힌 묶음도 기다리는 줄은 그대로 그리고, 머리글이 감춘 것 중 몇이 나를 부르는지 말한다. 다른 캔버스의
...
**`Ctrl −` 는 캔버스를 넓혀 준다.** 브라우저 자체의 축소라 글자 칸이 줄고, 모든 터미널이 한꺼번에 더
많은 행과 열을 갖는다.
...
멎으면 뗀다. palmer 는 PTY 의 주인이라 그 바이트를 그대로 보고, 거기서 상태를 읽는다. 설치할 것도,
설정 파일도, 받아야 할 허락도 없다. **palmer 가 배운 적 없는 에이전트에서도 켜진다.**
...
턴이 끝난 것을 가릴 수 있다. 훅은 스스로 붙는다. palmer 가 여는 모든 셸의 `PATH` 앞에 작은 껍데기가
놓이므로 여기서도 사용자가 할 일은 없다. zsh 와 bash 에서 된다.

**화면을 palmer 가 갖고 있지 않을 때는 알림이 찾아온다.** 브라우저 탭이 나를 부르는 개수를 세고,
...
| | |
|---|---|
| 데몬 | 파이썬 표준 라이브러리만 — PTY, HTTP, 웹소켓, 훅. 한 파일, 약 1,700줄. |
| 파이썬 | 3.9 이상. |
| 브라우저 | 순수 자바스크립트. 프레임워크도 빌드 단계도 없다. 터미널은 xterm.js 가 그린다. |
| 도는 곳 | macOS, 리눅스, WSL(데몬을 WSL 안에서 띄우고 윈도우 브라우저로 연다). |
| 보안 | `127.0.0.1` 에만 묶인다. 무언가를 바꾸는 요청마다 `0600` 파일의 토큰과 `Origin`·`Host` 검사. |

## 아직 안 만든 것

- **창이 서로 밀어내지 않는다.** 새 창은 빈 격자 자리에 놓이고, 창을 끌어 다른 창에 겹치면 겹친다.
- **캔버스를 지우는 단추도, 터미널을 다른 캔버스로 옮기는 단추도 없다.** 데몬은 둘 다 하고 열린
  브라우저가 모두 따라오지만, 그 손잡이가 화면 어디에 붙어야 하는지가 아직 안 정해졌다.
- **데몬을 다시 띄우면 아무것도 안 남는다** — 캔버스도, 이름도, 셸도.
...
palmer 는 셸만 연다 — 도구 목록도 없고, 무엇을 돌릴지에 대한 의견도 없다. 데몬은 PTY 와 캔버스와
...
살아 있는 목록은 [이슈](https://github.com/maengyo/palmer/issues)다.
